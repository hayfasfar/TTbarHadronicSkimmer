...
- Plots in `plots/images/toptag_wp/<iov>/`:
  ...
  - `data_mc_score_distributions.png` — data points over stacked QCD+TTbar
    ...
    in the input accumulator. This plot uses the full preselected `mSD` range,
    including the 2D alphabet sidebands. The full stacked MC is scaled to the
    data integral in each pT panel for a shape comparison, while the nominal
